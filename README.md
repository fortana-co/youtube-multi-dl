...
It tags them (title, artist, album, track number) so they're correctly grouped and ordered in your music player, cleans up the song titles, and gives files clean names. Output is Opus by default, or mp3. Opus has excellent quality at small sizes, and copied straight from YouTube's stream without re-encoding when possible.
...
`retag` points at an existing `<artist>/<album>` directory, rewrites the `artist`/`album` tags on its `.opus`/`.mp3` files, and moves the `<artist>/<album>` folder to match, leaving titles, track numbers, etc untouched. It refuses with `INVALID_ARGS` if the directory has no audio files, or if the destination already exists (you probably already have the corrected album there). Anything more involved than this is a job for a real library manager like [beets](https://beets.io/).
